...
for key in map:
    ...
    do(Clicker('비밀작전'))
    do(Clicker('비밀작전입장'))
    do(Clicker('비밀작전입장2', threshold=0.84))
    # do(Clicker('비밀작전입장3'))
    do(Clicker('비밀작전_제국'))
    do(Clicker('비밀작전_노페2'))
    do(Clicker('비밀작전_입장'))
    ...
    while True:
        ...
        while True:
            ...
            if (findBoss is False and do(Founder('비밀작전_보스발견', threshold=0.9), onlyOneTime=True)):
            ...
            if(do(Founder('던전재도전하기'), screenShot=screenShot, onlyOneTime=True)):
                ...
                if(do(Founder('피로도가부족합니다'), onlyOneTime=True, canSkip=True)):
                    if(do(Clicker('마을로가기', threshold=0.90))):
                        endLoop = True  
                        break
                if(do(Founder('던전재도전하기'), screenShot=screenShot, onlyOneTime=True) is False):
                    pyautogui.keyDown('x')
                    time.sleep(3)
                    pyautogui.keyUp('x')
                    ...
                    do(Clicker('던전재도전하기'), canSkip=True)
                    if(do(Founder('피로도가부족합니다'))):
                        do(Founder('마을로가기'))
                    break
                break 
