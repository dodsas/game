...
def 캐릭터선택2():
    ...
    for i in range(100):
        ...
        if do(Clicker('캐릭_' + char.name, threshold=0.86), onlyOneTime=True) :
            break
...
def 산등최초입장2():
    ...
    do(Clicker('입장_설산'))
    do(Clicker('모험난이도'))
    do(Clicker('산등성이', threshold=0.99))
    do(Clicker('전투시작'))
...
def 산등노가다2():
    ...
    while(True):
        j+=1
        uprint(char, "산등 노가다 진행중 : " + str(j) + "/" + str(loopCount))
        do(Founder('산등', threshold=0.95))
        robot.pressKey(str(char.buffIndex), sleep=0.1)
        do(Direct(1887, 467))
        do(Founder('맵_보스'))

        if(do(Founder('지옥파티'), onlyOneTime=True)): 
            j-=1
            if(j == loopCount):
                pyautogui.press("ESC")
                imageFinder.waitAndClick('재도전확인')
        ...
        for i in range(100):
            ...
            if(i%45 == 0 and i != 0):
                ...
                robot.pressKey(switcher.get(random.randint(0,2)), duration=2)
                do(Clicker('부활', screenShot=screenShot, threshold=0.75), onlyOneTime=True)

            pyautogui.keyDown('x')
            pyautogui.sleep(4)
            # action(Clicker('산등_골카', screenShot=screenShot), onlyOneTime=True)
            if(do(Founder('재도전', screenShot=screenShot, threshold=0.75), onlyOneTime=True)):
                pyautogui.keyUp('x')
